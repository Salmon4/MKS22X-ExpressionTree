public class ExpressionTree{
  ...
  public String toString(){
    String ans = "";
    if (this.isOp()){
      ans += "(" + this.getLeft().toString() + " " + this.getOp() + " " + this.getRight().toString()+ ")";
    }
    else{
      ans += this.getValue();
    }
  ...
  public String toStringPostfix(){
    String ans = "";
    if (this.isOp()){
    //  if (this.getLeft().isOp()){
    //  /  ans += this.getRight().toStringPostfix() + " " + this.getLeft().toStringPostfix() + " " + this.getOp();
    //  }
    //  else{
        ans += this.getLeft().toStringPostfix() + " " + this.getRight().toStringPostfix() + " " + this.getOp();
    //  }
    }
    else{
      ans += this.getValue();
  ...
  public String toStringPrefix(){
    String ans = "";
    if (this.isOp()){
      ans += this.getOp() + " " + this.getLeft().toStringPrefix() + " " + this.getRight().toStringPrefix();
    }
    else{
      ans += this.getValue();
    }
  ...
  public double evaluate(){
    double ans = 0.0;
    if (this.isOp()){
      char oper = this.getOp();
      if (oper == '+'){
        ans += this.getLeft().evaluate() + this.getRight().evaluate();
      }
      if (oper == '-'){
        ans += this.getLeft().evaluate() - this.getRight().evaluate();
      }
